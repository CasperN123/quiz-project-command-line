...
--
-- Host: localhost
-- Generation Time: Jun 15, 2018 at 06:51 AM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `AdvListQuizQuestionsAnswersAndUsersSorted`  AS  select `Q`.`title` AS `Quiz:`,`Q`.`owner` AS `Made by:`,`Questions`.`text` AS `Question:`,`A`.`rightAnswer` AS `Correct answer:`,`UA`.`user` AS `User:`,`UA`.`UserAnswer` AS `Answered:` from (((`Questions` join `QViewFix` `Q`) join `AViewFix` `A`) join `UAViewFix` `UA`) where (`Questions`.`questionID` = `UA`.`QID`) order by `Q`.`title`,`Q`.`owner`,`Questions`.`text`,`UA`.`user` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `AViewFix`  AS  select `Answers`.`answerText` AS `rightAnswer` from (`Answers` join `Questions` on(((`Answers`.`answerLocalID` = `Questions`.`trueCorrectAnswerID`) and (`Questions`.`questionID` = `Answers`.`questionID`)))) ;
